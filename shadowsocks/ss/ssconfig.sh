...
main_url="http://192.168.50.110:6999"
backup_url="https://raw.githubusercontent.com/zcr268/shadowsocks/master"
...
update_ss(){
	...
	ss_basic_version_web1=`curl --connect-timeout 5 -s "$main_url"/version | sed 's/ /\n/g'| sed -n 1p`
	if [ -n "$ss_basic_version_web1" ];then
		...
		echo_date 主服务器在线版本号："$ss_basic_version_web1" 和本地版本号："$ss_basic_version_local" 不同！
			cd /tmp
			md5_web1=`curl -s "$main_url"/version | sed 's/ /\n/g'| sed -n 2p`
			...
			if [ "$md5sum_gz" != "$md5_web1" ]; then
				...
				echo_date 本地MD5 $md5sum_gz
				echo_date 远程MD5 $md5_web1
...
update_ss2(){
	...
	ss_basic_version_web2=`curl --connect-timeout 5 -s "$backup_url"/version | sed 's/ /\n/g'| sed -n 1p`
	...
	echo_date 检测到备用服务器在线版本号：$ss_basic_version_web1
		...
		echo_date 备用服务器在线版本号："$ss_basic_version_web1" 和本地版本号："$ss_basic_version_local" 不同！
			cd /tmp
			md5_web2=`curl -s "$backup_url"/version | sed 's/ /\n/g'| sed -n 2p`
